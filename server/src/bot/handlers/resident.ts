...
async function showMy(ctx: BotContext): Promise<void> {
  if (!isDialog(ctx)) return;
...
async function showSupported(ctx: BotContext): Promise<void> {
  if (!isDialog(ctx)) return;
  if (!(await ensureOnboarded(ctx, null))) return;
  await ack(ctx);
...
async function showContacts(ctx: BotContext): Promise<void> {
  if (!isDialog(ctx)) return;
  await ack(ctx);
