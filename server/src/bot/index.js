...
export function createBot() {
  ...
  // 1. Сессии (в PostgreSQL) — нужны для пошаговых сценариев
  bot.use(session({ store: new PrismaSessionStore(), defaultSession: () => ({}) }));

  // 2. Пользователь из нашей БД
  ...
  // 3. Сценарии: контроллер → глобальные команды → перехват активного сценария → остальные обработчики
  ...
  // 4. Непонятный текст в личном диалоге
...
/** Получает профиль бота, подключает уведомления и регистрирует список команд. */
export async function prepareBot(bot) {
