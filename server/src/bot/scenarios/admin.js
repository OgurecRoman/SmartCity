...
// ---------- Добавление / удаление владельца ----------

export const manageOwnerScenario = defineScenario()({
  id: 'manage-owner',
  initialStep: 'start',
...
// ---------- Объявление в чат дома ----------

export const announceScenario = defineScenario()({
  id: 'announce',
  initialStep: 'start',
  idleTimeoutMs: SCENARIO_TIMEOUT_MS,
  createData: () => ({}),
  ...
  steps: {
    ...
    text: async ({ ctx }) => {
      ...
      await ctx.reply(
        ...
        withKeyboard([[btn.callback('Отправить', 'ann:yes'), btn.callback('Отмена', 'cancel')]]),
      );
...
// ---------- Передача заявки в организацию ----------

async function refreshCard(ctx, requestId, cardMid, note) {
  if (!cardMid) return;
  ...
  try {
    ...
    // карточка могла быть удалена — это не критично
  }
}

export const delegateScenario = defineScenario()({
  id: 'delegate-request',
  initialStep: 'start',
  idleTimeoutMs: SCENARIO_TIMEOUT_MS,
  intercept: cancelIntercept,
  steps: {
    start: async ({ ctx, data }) => {
      ...
      if (organizations.length === 0) {
        await ctx.reply(
          'Справочник организаций пуст. Добавьте организации в БД (таблица ResponsibleOrganization).',
          withKeyboard(panelButton()),
        );
        return transition.cancel();
      }
      ...
      const rows = sorted.map((org) => [
        btn.callback(org.categories.includes(request.category) ? `⭐ ${org.name}` : org.name, `dlg:org:${org.id}`),
      ]);
...
export const rejectScenario = defineScenario()({
  id: 'reject-request',
  initialStep: 'start',
  idleTimeoutMs: SCENARIO_TIMEOUT_MS,
  intercept: cancelIntercept,
