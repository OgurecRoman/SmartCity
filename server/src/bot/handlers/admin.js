...
import { keyboard, panelButton, requestCard, ukRequestButtons } from '../ui.js';
...
async function setStatus(ctx, requestId, status, note) {
  if (!(await guard(ctx))) return;
  try {
...
export function registerAdminHandlers(bot) {
  ...
  bot.action(/^uk:take:(\d+)$/, (ctx) =>
    setStatus(ctx, Number(ctx.match?.[1]), 'IN_PROGRESS', '🛠 Заявка взята в работу. Жители уведомлены.'),
  );
  bot.action(/^uk:resolve:(\d+)$/, (ctx) =>
    setStatus(ctx, Number(ctx.match?.[1]), 'RESOLVED', '✅ Заявка закрыта как выполненная. Жители уведомлены.'),
  );
  ...
  const startOwner = (action) => async (ctx) => {
    if (!(await guard(ctx))) return;
    await ack(ctx);
