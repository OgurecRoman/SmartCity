...
import { chatDetailsButtons, keyboard, requestCard, ukRequestButtons, withKeyboard } from './ui.js';

let api = null;
let subscribed = false;

async function sendDm(maxUserId, text, extra) {
  if (!api) return;
  try {
    await api.sendMessageToUser(Number(maxUserId), text, extra);
  } catch (error) {
    // Пользователь мог не запускать бота — тогда личное сообщение доставить нельзя
...
async function editMessage(messageId, text, rows) {
  if (!api) return;
  try {
...
/** Обновляет сообщение-уведомление в чате дома (счётчик подписей, статус). */
async function refreshChatMessage(request, note) {
...
async function notifyEmployees(request, heading) {
...
async function voterIds(requestId, excludeUserId) {
...
function subscribe() {
  ...
  events.on('request.submitted', async ({ requestId }) => {
    ...
    await Promise.all(voters.map((id) => sendDm(id, `📨 Заявка №${request.id} «${request.title}», которую вы поддержали, передана в УК.`)));
...
/** Подключает рассылку уведомлений к API бота. Вызывать один раз после запуска бота. */
export function initNotifications(botApi) {
  api = botApi;
  subscribe();
}
