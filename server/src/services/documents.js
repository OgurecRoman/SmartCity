...
/** Текстовый документ по заявке: реквизиты, описание, подписи, история статусов. */
export function buildRequestDocument(request) {
  const lines = [];
  ...
  for (const entry of request.statusHistory) {
    ...
    lines.push(
      `  ${formatDateTime(entry.changedAt)}: ${from}${STATUS_LABELS[entry.newStatus]} (${who})${entry.comment ? ` — ${entry.comment}` : ''}`,
    );
  }
  lines.push('');
