/**
 * Офлайн-прогон сценариев бота без MAX: API бота подменяется заглушками, обновления подаются вручную.
 * Нужна рабочая БД (DATABASE_URL). Запуск: npm run bot:simulate
 */
...
bot.botInfo = {
  user_id: BOT_ID,
  first_name: 'SmartCity',
  name: 'SmartCity',
  username: 'smartcity_demo_bot',
  is_bot: true,
...
const settle = () => new Promise((resolve) => setTimeout(resolve, 150)); // уведомления рассылаются асинхронно
...
async function msg(u, text, chatId = dm(u), chatType = 'dialog') {
  ...
  await handle({
    ...
    message: {
      sender: u,
      recipient: { chat_id: chatId, chat_type: chatType, user_id: null, post_id: null },
      timestamp: Date.now(),
      body: { mid: `in_${midCounter}`, seq: midCounter, text },
    },
  });
  await settle();
}
async function cb(u, payload, chatId = dm(u), chatType = 'dialog') {
  ...
  await handle({
    ...
    message: {
      sender: { ...u, user_id: BOT_ID, is_bot: true },
      recipient: { chat_id: chatId, chat_type: chatType, user_id: null, post_id: null },
      timestamp: Date.now(),
      body: { mid: `card_${midCounter}`, seq: midCounter, text: '(карточка)' },
    },
  });
  await settle();
}
...
async function onboard(u, apartment, type) {
  await cb(u, 'onb:house:1');
  await msg(u, apartment);
  await cb(u, `onb:type:${type}`);
}
async function latestRequestId(maxUserId) {
