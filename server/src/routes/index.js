...
import { CATEGORY_LABELS, CATEGORY_ORDER, PRIORITY_LABELS, RESIDENT_TYPE_LABELS, STATUS_LABELS, parseRuDate } from '../lib/labels.js';
...
const REQUEST_STATUSES = Object.keys(STATUS_LABELS);
const REQUEST_CATEGORIES = Object.keys(CATEGORY_LABELS);

// Справочники (без авторизации — нужны фронтенду до входа)
apiRouter.get('/dictionaries', (_req, res) => {
  res.json({
    ...
    priorities: Object.keys(PRIORITY_LABELS).map((value) => ({ value, label: PRIORITY_LABELS[value] })),
    residentTypes: Object.keys(RESIDENT_TYPE_LABELS).map((value) => ({
      value,
...
// Экран выбора дома на карте: здание в точке нажатия (OpenStreetMap) и дом из базы, если он уже подключён
...
// Житель подтвердил дом на карте: вернуть существующий дом или создать новый (201)
...
// Поиск адреса по строке для карты мини-приложения (OpenStreetMap Nominatim)
...
const statusSchema = z.object({
  status: z.enum(UK_SETTABLE_STATUSES),
...
apiRouter.patch('/requests/:id/status', requireEmployee, async (req, res) => {
  ...
  const request = await changeStatus(requestId, input.status, {
    byUserId: user.id,
