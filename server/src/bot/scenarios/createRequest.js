...
export const createRequestScenario = defineScenario()({
  id: 'create-request',
  initialStep: 'start',
  ...
  steps: {
    ...
    category: async ({ ctx }) => {
      ...
      const category = match && match[1] in CATEGORY_LABELS ? match[1] : null;
      if (!category) {
    ...
    description: async ({ ctx }) => {
      ...
      await ctx.reply(
        ...
        withKeyboard([
          [btn.callback('Обычная заявка', 'cr:prio:NORMAL')],
          [btn.callback('🚨 Аварийная', 'cr:prio:EMERGENCY')],
